...
class RentService:
    ...
    def rent_for_location(self, listing: PropertyListing, percentile: int, adu_unit: RentalUnit = None,
                         cache_only=True) -> int:
        ...
        :param RentalUnit adu_unit: if set, return the value for an ADU of adu_unit at the location
        ...
        units: [RentalData] = [adu_unit] if adu_unit else listing.parcel.rental_units
        data_type = RentalData.RentalDataType.ADU_RENTOMETER_ESTIMATE if adu_unit else RentalData.RentalDataType.UNIT_RENTOMETER_ESTIMATE
        ...
        if not adu_unit and (listing.parcel.br < 1 or listing.parcel.ba < 1):
        ...
        for unit in units:
            ...
            if rd.details['samples'] >= 10:
                ...
                if maybe_rent < rd.details['max']:
                    rents[unit] = maybe_rent
            if rents[unit] == -1:
                # we didn't manage to assign a rent using the normal distribution, so fall back to a linear model
                assert (percentile >= 50)
                if percentile > 75:
                    rents[unit] = rd.details['percentile_75'] + ((percentile - 75) * (rd.details['max'] - rd.details['percentile_75']))/25
                else:
                    rents[unit] = rd.details['mean'] + ((percentile - 50) * (rd.details['percentile_75'] - rd.details['mean'])) / 25
            assert (rents[unit] < rd.details['max'])
            cum_rents += rents[unit]
        return int(cum_rents)
