...
class RentService:
    ...
    def interpolate_rent_for_location(
        self, listing: PropertyListing, units: [RentalUnit], percentile: int, is_adu=False
    ):
        from django.contrib.gis.db.models.functions import Distance
        from django.contrib.gis.measure import D

        loc = self.listing.parcel.geom.centroid
        near_rents = RentalData.objects.filter(location__distance_lte=(loc, D(m=1000.0))).annotate(
    ...
    def rent_for_location(
        ...
        for unit in units:
            ...
            if not len(x) and cache_only:
                log.info(
                    f"Skipping {check_br}BR, {check_ba}BA at {listing.addr}, mlsid={listing.mlsid}, APN={listing.parcel.apn}"
            ...
            if rent_cache[unit] == -1:
                ...
                if percentile > 75:
                    rent_cache[unit] = (
                        ...
                        + ((percentile - 75) * (rd.details["max"] - rd.details["percentile_75"]))
                        / 25
                    )
                else:
                    rent_cache[unit] = (
                        rd.details["mean"]
                        + ((percentile - 50) * (rd.details["percentile_75"] - rd.details["mean"]))
                        / 25
                    )
