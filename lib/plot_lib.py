...
def plot_split_lot(
    ...
    geopandas.GeoSeries(second_lot).plot(ax=ax, color="cyan", alpha=0.7)
...
def plot_cant_build(
    ...
    for poly in setbacks:
        if not poly.is_empty:
            geopandas.GeoSeries(poly).plot(ax=ax, color="orange", alpha=0.7)

    if flag_poly:
        geopandas.GeoSeries(flag_poly).plot(ax=ax, color="green", alpha=0.3)
