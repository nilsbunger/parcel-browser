...
class MailerSendBackend(BaseEmailBackend):
    ...
    def send_messages(self, email_messages: list[django.core.mail.message.EmailMessage]) -> str:
        ...
        mailer_cc_list = [{"name": cc_item, "email": cc_item} for cc_item in email_message.cc]
        mailer_bcc_list = [{"name": bcc_item, "email": bcc_item} for bcc_item in email_message.bcc]
        mailer.set_mail_to(mailer_to_list, mail_body)
        mailer.set_cc_recipients(mailer_cc_list, mail_body)
        mailer.set_bcc_recipients(mailer_bcc_list, mail_body)
        ...
        print(f"Sent email to:{email_message.to}, cc:{email_message.cc}, bcc:{email_message.bcc}")
        return resp
