...
from world.models import WorldBorder, parcel_mapping, world_mapping, Parcel, ZoningBase, zoningbase_mapping, \
    BuildingOutlines, buildingoutlines_mapping, AnalyzedParcel
...
class Command(BaseCommand):
    ...
    def histo(self):
        ...
        for bucket in histo_buckets:
            with connection.cursor() as cursor:
                cursor.execute("select count(lot_size) from world_analyzedparcel where lot_size >= %s and skip is false" % bucket)
                row = cursor.fetchone()
            print(row[0])
    ...
    def rebuild(self):
        ...
        selfields = "apn, world_parcel.id, usable_sq_field, total_lvg_field, acreage, zone_name, ordnum, nucleus_us, situs_juri, OVERLAY_JU"
