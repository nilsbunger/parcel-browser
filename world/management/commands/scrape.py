...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        unlinked_listings = PropertyListing.objects.filter(status='ACTIVE')
        stats = defaultdict(int)
        print(f'Found {len(unlinked_listings)} properties without an associated parcel')
        for l in unlinked_listings:
