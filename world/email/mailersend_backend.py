...
class MailerSendBackend(BaseEmailBackend):
    ...
    def send_messages(self, email_messages: List[django.core.mail.message.EmailMessage]) -> str:
        ...
        assert len(email_message.to) == 1, "We only support sending to one recipient at the moment"
        ...
        mailer.set_mail_to([{"name": email_message.to[0], "email": email_message.to[0]}], mail_body)
        ...
        print("Sent email to", email_message.to[0])
        return resp
