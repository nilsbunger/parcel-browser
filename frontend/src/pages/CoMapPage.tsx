...
const TILE_DEFS2 = (zoneColorFn, visibleLayers) => {
  return {
    // 'road-tile-layer': {
    //   data: `${BACKEND_DOMAIN}/dj/api/roadtile/{z}/{x}/{y}`,
    ...
    // },
    // 'parcel-tile-layer': {
    //   data: `${BACKEND_DOMAIN}/dj/api/parceltile/{z}/{x}/{y}`,
    //   getLineColor: [128, 128, 128],
    ...
    "tpa-tile-layer": {
      data: `${BACKEND_DOMAIN}/dj/api/tpatile/{z}/{x}/{y}`,
    ...
    "compcomm-tile-layer": {
      data: `${BACKEND_DOMAIN}/dj/api/compcommtile/{z}/{x}/{y}`,
      getLineColor: [255, 255, 255],
    ...
    "zoning-tile-label-layer": {
      data: `${BACKEND_DOMAIN}/dj/api/zoninglabeltile/{z}/{x}/{y}`,
      getLineColor: [128, 128, 128],
      minZoom: 15,
      maxZoon: 15,
      lineWidthMinPixels: 2,
    ...
    "zoning-tile-layer": {
      data: `${BACKEND_DOMAIN}/dj/api/zoningtile/{z}/{x}/{y}`,
      getLineColor: [128, 128, 128],
    ...
    "ab2011-tile-layer": {
      data: `${BACKEND_DOMAIN}/dj/api/ab2011tile/{z}/{x}/{y}`,
      getLineColor: [255, 255, 255],
      ...
      >
        ...
        <Menu.Dropdown>
          ...
          <Menu.Item onClick={(e) => toggleLayer(e, "sf-vis-layer")}>
            <LayerSquare enabled={visibleLayers["sf-vis-layer"]} color={LAYER_COLORS["sf-vis-layer"]} />
            Single-family
          </Menu.Item>
          <Menu.Item onClick={(e) => toggleLayer(e, "mf-vis-layer")}>
            <LayerSquare enabled={visibleLayers["mf-vis-layer"]} color={LAYER_COLORS["mf-vis-layer"]} />
            Multi-family
          </Menu.Item>
          <Menu.Item onClick={(e) => toggleLayer(e, "pd-vis-layer")}>
            <LayerSquare enabled={visibleLayers["pd-vis-layer"]} color={LAYER_COLORS["pd-vis-layer"]} />
            Planned Districts
          </Menu.Item>
          {/*<Menu.Item rightSection={<Text size="xs" color="dimmed">⌘K</Text>}>Search</Menu.Item>*/}
          <Menu.Item onClick={(e) => toggleLayer(e, "c-vis-layer")}>
            <LayerSquare enabled={visibleLayers["c-vis-layer"]} color={LAYER_COLORS["c-vis-layer"]} />
            Commercial zones
          </Menu.Item>
          <Menu.Item></Menu.Item>
          <Menu.Divider />
          ...
          <Menu.Item onClick={(e) => toggleLayer(e, "compcomm-vis-layer")}>
            <LayerSquare
              enabled={visibleLayers["compcomm-vis-layer"]}
              color={LAYER_COLORS["compcomm-vis-layer"]}
            />
...
export default function CoMapPage({ onTilesLoad = null }) {
  ...
  const zonesVisible =
    visibleLayers["c-vis-layer"] ||
    visibleLayers["pd-vis-layer"] ||
    visibleLayers["sf-vis-layer"] ||
    visibleLayers["mf-vis-layer"]
  return (
