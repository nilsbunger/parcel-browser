...
const topCheck = (text, state, onChangeFn) => (
  <div key={text} className="form-control w-36">
    <label className="cursor-pointer label justify-start gap-3">
      {" "}
      {/* Label is actually a flex parent */}
      <input
        type="radio"
        value={state === text ? "true" : ""}
        checked={state === text}
        name="unitfilter"
      ...
      <Tooltip permanent={false}>
        ...
        {Object.entries(rents).map((kv: [string, UnitRentData], idx) => (
          <p key={"rentalRate.pid" + kv[0]}>
            {kv[0]} : Mean: ${kv[1].rent_mean.toLocaleString()}. 75th percentile: $
            {kv[1].rent_75_percentile.toLocaleString()}. {kv[1].num_samples} samples
          </p>
        ))}
      </Tooltip>
    </Marker>
